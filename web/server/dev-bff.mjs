...
function setCookie(res, value) {
  res.setHeader('Set-Cookie',
    `${COOKIE}=${encodeURIComponent(value)}; HttpOnly; Path=/; SameSite=Lax; Max-Age=86400`);
}
...
app.post('/api/dev/logout', (_req, res) => { clearCookie(res); res.json({ ok: true }); });
...
app.post('/api/auth/otp/send', async (req, res) => {
  ...
  try {
    const userId = await findUserIdByPhone(pool, normalized);
    if (!userId) return res.status(404).json({ error: 'unknown_phone' });
    const requiresMfa = await userRequiresMfa(pool, userId);
    const challenge = issueOtpChallenge(normalized, userId, requiresMfa);
...
app.post('/api/auth/otp/verify', async (req, res) => {
  ...
  const normalized = normalizePhone(phone);
  if (!normalized) return res.status(400).json({ error: 'invalid_phone' });
  if (!validateOtpCode(otp)) return res.status(401).json({ error: 'invalid_otp' });
  const challenge = consumeOtpChallenge(normalized);
  if (!challenge) return res.status(401).json({ error: 'otp_expired' });
  if (challenge.requiresMfa && !validateMfaCode(mfaCode)) {
    return res.status(401).json({ error: 'invalid_mfa' });
  }
  try {
    const session = await buildSession(challenge.userId);
    if (!session) return res.status(404).json({ error: 'unknown_user' });
    setCookie(res, challenge.userId);
    res.json(session);
  } catch (err) {
...
app.get('/api/me', async (req, res) => {
  ...
  const profile = await lookupProfile(userId);
  if (!profile) { clearCookie(res); return res.status(401).json({ authenticated: false }); }
...
app.listen(PORT, () => console.log(`dev BFF listening on http://127.0.0.1:${8787}`));
